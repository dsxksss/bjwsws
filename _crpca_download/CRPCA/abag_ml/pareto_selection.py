...
# --- BEGIN PATCH (optional imports) ---
# 这些 import 依赖未随本仓库分发的模块（vaccine_advance_core.featurization.seq_to_features /
# bio_sequence_manipulation 及 abag_agent_setup.* → 进一步牵连 vaccine_advance_core_io /
# statium_automation / improvwf 等缺失文件）。本模块被 local_pipeline/run_pipeline.py 使用时，
# 仅调用其中的 Pareto 选择函数（get_pareto_rows / simple_row_scorer / simple_negative_row_scorer /
# apply_dominance_function_naive 等），不需要下列名称。改为可选导入，缺失时不阻断本模块加载。
try:
...
except ImportError as _opt_imp_err:  # pragma: no cover
    AllowedMutationsKey = derive_idstr_from_seq = _override_master_mutant = None
    override_single_point_df = generate_linear_mutants = None
    diff_seqs = mutate_seq = recast_sequence_as_str = None
    warnings.warn(
        "abag_ml.pareto_selection: 可选的 mutant-menu 导入不可用 ({}); "
        "仅 Pareto 选择相关函数可用。".format(_opt_imp_err))
# --- END PATCH ---
